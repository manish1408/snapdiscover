...
export default function DetailProduct() {
  ...
  const { data: product, loading } = useFetchCollectionDetails(
    productId,
    "products"
  );
  ...
  const shareOnInstagram = async () => {
    try {
      const isInstagramInstalledAndroid = await Share.isPackageInstalled(
        "com.instagram.android"
      );
      const isInstagramInstalledIOS = await Linking.canOpenURL(
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.container}>
        ...
        <View style={styles.containerCantSold}>
          <View style={styles.sold}>
            <Typography translate={false}>
              {product?.additionalInfo?.breweryInfo?.name}
            </Typography>
        ...
        {product?.additionalInfo?.similarProducts && (
          <View style={styles.containerDescription}>
            ...
            <RelatedProduct
              similarProducts={product?.additionalInfo?.similarProducts}
            />
          </View>
        )}
          ...
          <View style={styles.containerSocial}>
            <TouchableOpacity onPress={() => shareOnSocial("FACEBOOK")}>
              <Image
                ...
                source={require("@/shared/assets/icons-8/facebook.png")}
              />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => shareOnSocial("WHATSAPP")}>
              <Image
                resizeMode="contain"
                style={styles.socialImage}
                source={require("@/shared/assets/icons-8/whatsapp.png")}
              />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => shareOnInstagram()}>
              <Image
                resizeMode="contain"
                style={styles.socialImage}
                source={require("@/shared/assets/icons-8/insta.png")}
              />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => shareOnSocial("TWITTER")}>
              <Image
                resizeMode="contain"
                style={styles.socialImage}
                source={require("@/shared/assets/icons-8/twitter.png")}
              />
            </TouchableOpacity>
          </View>
        </View>

        {product?.additionalInfo?.breweryInfo &&
          Object.keys(product?.additionalInfo?.breweryInfo).length > 0 && (
            <View style={styles.containerDescription}>
              <Typography style={styles.descriptionTitle}>
                Visit Brewery
              </Typography>
