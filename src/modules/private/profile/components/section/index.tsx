...
import { arrowRight } from '@/shared/assets/icons-8';
	...
	return (
		...
		>
			...
			{elements.map((element, index) => (
				<TouchableOpacity key={index} style={styles.rowContainer} onPress={element?.onPress}>
					<View style={{ flexDirection: 'row', alignItems: 'center' }}>
						{element.leftIcon}
						<Typography style={styles.name}>{element.name}</Typography>
					</View>
					{element.rightElement ? element.rightElement : <Icon customStyles={stylesIcon} icon={arrowRight} />}
				</TouchableOpacity>
			))}
		</View>
	);
